...
import { DownloadWorkerType, IDownloadOptions } from './types/IDownload';
...
import { DownloadWorkerExperimental } from './DownloadWorkerExperimental';

import { AlreadyDownloaded } from './types/Errors';
...
class DownloadManager {
  ...
  private mBusyWorkers: { [id: number]: IDownloadWorker } = {};
  ...
  private mExperimentalDownloader: () => boolean;
  ...
  constructor(downloadPath: string, maxWorkers: number, maxChunks: number,
    ...
    this.mExperimentalDownloader = experimentalDownloader;
    this.mApi = api;
  }
  ...
  public enqueue(id: string, urls: string[],
                 ...
                 // find out why we can't install collections/ also remember that the experimental downloader is kicking off incorrectly when attempting to resume downloads.
                 options?: IDownloadOptions): Bluebird<IDownloadResult> {
    const workerType: DownloadWorkerType = options?.downloadWorkerType ?? this.mExperimentalDownloader() ? 'experimental' : 'regular';
    if (urls.length === 0) {
    ...
    return fs.ensureDirAsync(destPath)
      ...
      .then((filePath: string) =>
        new Bluebird<IDownloadResult>(async (resolve, reject) => {
          const origName = nameTemplate;
          ...
          download = {
            ...
            origName,
            tempName,
            finalName: Bluebird.resolve(finalName),
            error: false,
            urls,
            resolvedUrls: this.resolveUrls(urls, nameTemplate, options?.nameHint),
            options: {
              ...options,
              downloadWorkerType: workerType,
            },
            started: new Date(),
            lastProgressSent: 0,
            received: 0,
  ...
  private startJob = (download: IRunningDownload, job: IDownloadJob) => {
    if (download.options?.downloadWorkerType === 'experimental' || this.mExperimentalDownloader()) {
      download.assemblerProm = Bluebird.resolve<FileAssembler>(null);
    }
    ...
    return download.assemblerProm.then(assembler => {
      ...
      const progressCB = this.makeProgressCB(job, download);
      this.mBusyWorkers[job.workerId] = (download.options.downloadWorkerType === 'experimental')
        // experimental downloader is not yet ready for prime time
        ? new DownloadWorkerExperimental(this.mApi, download, job, this.mDownloadPath, progressCB,
          (pause, replaceFileName) => replaceFileName !== undefined
            ? this.useExistingFile(download, job, replaceFileName)
            : this.finishExperimentalChunk(download, job, pause))

        // use the old downloader
        : new DownloadWorker(job, progressCB,
          (pause, replaceFileName) => replaceFileName !== undefined
            ? this.useExistingFile(download, job, replaceFileName)
  ...
  private makeDataCB = (download: IRunningDownload) => {
    return (offset: number, data: Buffer) => {
      ...
      const workerType = download.options.downloadWorkerType ?? this.mExperimentalDownloader() ? 'experimental' : 'regular';
      if (workerType === 'regular' && download.assembler?.isClosed?.()) {
        return Bluebird.reject(new ProcessCanceled('file already closed'));
      }
      return ((workerType === 'experimental')
      ...
      : download.assembler?.addChunk?.(offset, data))
        .then((synced: boolean) => {
          ...
          download.received = workerType === 'experimental' ? offset : data.byteLength;
  ...
  private finishExperimentalChunk = async (download: IRunningDownload, job: IDownloadJob, paused: boolean) => {
    this.stopWorker(job.workerId);
    let finalPath = download.tempName;
    await download.assembler?.close?.();
    if (download.finalName !== undefined) {
      const resolvedPath = await download.finalName;
      finalPath = resolvedPath;
      const received = download.size;
      ...
      if (download.tempName !== resolvedPath) {
        ...
        await fs.renameAsync(download.tempName, resolvedPath);
      }
      const resolved = await download.resolvedUrls().catch(() => ({ urls: [], meta: {} }));
      // const unfinishedChunks = download.chunks.filter(chunk => chunk.state === 'paused').map(this.toStoredChunk);
      download.finishCB({
        filePath: finalPath,
        headers: download.headers,
        unfinishedChunks: [],
